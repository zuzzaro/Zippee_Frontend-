...
function PartnerTripEntryForm() {
    ...
    const convertSvgToImage = (svgElement) => {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                canvas.width = 256;  // Set fixed width
                canvas.height = 256; // Set fixed height
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            // Wait for QR code to be generated and converted
    ...
    const handlePrint = async () => {
        try {
            // Wait for QR code to be generated and converted if needed
            if (!qrCodeDataUrl) {
                await new Promise(resolve => setTimeout(resolve, 500));
            }
